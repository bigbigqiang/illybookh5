...
export default {
  ...
  activated () {
    this.resetData()
    if (this.$route.query.type === 'book' && this.$route.query.bookCode) {
  ...
  methods: {
    ...
    getAllData () {
      ...
      this.$axios.all([this.getBookByCode(), this.getUserInfo(), this.getCouponList()])
        .then(this.$axios.spread((bookInfo, userInfo, coupons) => {
